...
- 共享一个棋局窗口做近实时复盘；每帧完整重识别，连续确认后才同步
...
- 每一帧都会从头识别完整棋盘，不会把上一帧的低置信度点拼进新结果。
- 单步合法变化需连续两帧一致，保留完整手顺；漏过多手或规则校验失败时，
  同一完整局面在四个不同视频帧中稳定至少 1.5 秒后，按当前画面自动重新同步。
- 手动截图确认过的局面会直接作为实时复盘基线，不需要重新猜测初始盘面。
